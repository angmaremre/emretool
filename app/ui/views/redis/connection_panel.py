"""Kayıtlı Redis bağlantılarını listeleyen sol panel."""
...
class RedisConnectionPanel(QWidget):
    ...
    def __init__(self, storage: Storage, parent: Optional[QWidget] = None) -> None:
        ...
        header = QLabel("Redis Bağlantıları")
    ...
    def refresh(self) -> None:
        ...
        for profile in self._storage.list_profiles(MODULE):
            item = QListWidgetItem(
                f"{profile.name}\n{profile.host}:{profile.port}/{profile.extra.get('db', 0)}"
            )
    ...
    def _new(self) -> None:
        if RedisConnectionDialog(self._storage, parent=self).exec():
            self.refresh()
    ...
    def _edit(self) -> None:
        ...
        if profile is None:
            return
        if RedisConnectionDialog(self._storage, profile=profile, parent=self).exec():
            self.refresh()
